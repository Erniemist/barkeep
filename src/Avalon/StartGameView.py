...
class StartGameView(discord.ui.View):
    ...
    async def select_players(self, interaction, select: discord.ui.UserSelect):
        return await interaction.response.send_message(f"Selected {', '.join([user.display_name for user in select.values])}!")
    ...
    async def select_roles(self, interaction, select: discord.ui.Select):
        if len(self.select_roles.values) != len(self.select_roles.values):
